...
PPCRecFunction_t* PPCRecompiler_recompileFunction(PPCFunctionBoundaryTracker::PPCRange_t range, std::set<uint32>& entryAddresses, std::vector<std::pair<MPTR, uint32>>& entryPointsOut, PPCFunctionBoundaryTracker& boundaryTracker, bool applyConfiguredRange = true)
{
	...
	if (applyConfiguredRange && ppcRecLowerAddr != 0 && ppcRecUpperAddr != 0)
	{
	...
	if (applyConfiguredRange && ActiveSettings::DumpRecompilerFunctionsEnabled())
	{
...
void PPCRecompiler_init()
{
	...
	PPCRecompiler_RunAArch64DifferentialTests();
	// Differential cases compile isolated functions before the worker starts.
	// Keep per-title telemetry limited to code generated by the title itself.
	PPCRecompiler_ResetStats();
#endif
