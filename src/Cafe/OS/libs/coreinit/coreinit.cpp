...
namespace
{
	...
	void DebugLogPanicPPCContext()
	{
		...
		constexpr uint32 kStackSnapshotSize = 0x80;
		...
		if (stackPointer < stackMinAddress || stackPointer > stackMaxAddress ||
			stackMaxAddress - stackPointer < kStackSnapshotSize ||
			!memory_isAddressRangeAccessible(stackPointer, kStackSnapshotSize))
		{
			cemuLog_log(LogType::Force, "PPC panic stack snapshot unavailable at {:08x}", stackPointer);
			return;
		}

		for (uint32 offset = 0; offset < kStackSnapshotSize; offset += 0x10)
		{
