...
- **R-10 — mitigado antes da primeira release:** o fork usa `io.github.rickamaral94.cemu`, nome próprio e `versionCode` monotônico. A chave dedicada será mantida offline; o CI comum não possui autoridade de release. Instalações experimentais antigas com `info.cemu.cemu` não são atualizadas no lugar e exigem migração/exportação explícita de dados.
